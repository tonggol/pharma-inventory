...
public interface StockTransactionRepository extends JpaRepository<StockTransaction, Long>, JpaSpecificationExecutor<StockTransaction> {
     ...
     * 입고 트랜잭션만 조회
     */
    @Query("SELECT st FROM StockTransaction st WHERE st.transactionType = com.pharma.inventory.entity.TransactionType.INBOUND")
    List<StockTransaction> findInboundTransactions();

    /**
     * 출고 트랜잭션만 조회
     */
    @Query("SELECT st FROM StockTransaction st WHERE st.transactionType = com.pharma.inventory.entity.TransactionType.OUTBOUND")
    ...
    @Query("""
                ...
                FROM StockTransaction st
                WHERE st.transactionType = com.pharma.inventory.entity.TransactionType.OUTBOUND
    ...
    @Query("""
                SELECT st
                FROM StockTransaction st
                WHERE st.transactionType = com.pharma.inventory.entity.TransactionType.DISPOSAL
                AND st.transactionDate BETWEEN :startDate AND :endDate
            """)
